...
@section('content')
    <div class="bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-gray-800 border-b border-gray-700">
            <div class="mb-6">
                <h2 class="text-2xl font-semibold text-gray-100">Tambah Buku Baru</h2>
                <p class="mt-1 text-sm text-gray-400">Silakan isi formulir di bawah untuk menambahkan buku baru ke dalam
                    database.</p>
            </div>
            ...
            <form method="POST" action="{{ route('books.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
